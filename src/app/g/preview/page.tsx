...
// ── SOFT-COMPARE — old vs recoloured <3 / teal on receipt paper ──────────────
// Preview-only: judge the softened doodle-math-heart + doodle-underline-teal
// against the real near-white paper, beside neighbour doodles. Renders the
// *-soft PNGs directly (NOT via the registry), so nothing is bound or
// overwritten — the live assets are untouched. view=softcompare
function SoftCompare() {
  const D = '/stickers/doodles';
  const PAPER = '#fbfbf9';
  const INK = '#141b2b';
  const cap: React.CSSProperties = { fontSize: 10.5, color: '#9a8f93' };
  /* eslint-disable @next/next/no-img-element */
  const heart = (soft: boolean) => (
    <img
      alt=""
      src={`${D}/doodle-math-heart${soft ? '-soft' : ''}.png`}
      style={{ height: 22, width: 'auto', verticalAlign: 'middle' }}
    />
  );
  const feelings = (soft: boolean) => (
    <span style={{ position: 'relative', display: 'inline-block' }}>
      feelings
      <img
        alt=""
        src={`${D}/doodle-underline-teal${soft ? '-soft' : ''}.png`}
        style={{
          position: 'absolute',
          left: '-8%',
          width: '116%',
  ...
  const chip = (file: string, h = 22) => (
    <img
      alt=""
      src={`${D}/${file}`}
      style={{ height: h, width: 'auto', verticalAlign: 'middle' }}
    />
  );
  ...
  return (
    ...
    >
      <h1 style={{ fontSize: 16, margin: '0 0 4px', color: '#1a1a1a' }}>
        Love Receipt — softened &lt;3 + teal (old vs new)
      </h1>
      <p
        style={{
          fontSize: 12,
      ...
      >
        Same brush texture, only the colour moved: &lt;3 crimson →{' '}
        <code>#F1A2B1</code> warm rose, teal → <code>#63B1AE</code> soft aqua.
        Shown on the real paper (#fbfbf9). Originals are untouched — nothing is
        committed to the live assets yet.
      </p>
      <div style={paper}>
        <div style={cap}>&lt;3 end-mark — CURRENT (crimson)</div>
        <div style={line}>
          <span>you finish my sentences</span>
          {heart(false)}
        </div>
        <div style={{ ...cap, marginTop: 10 }}>&lt;3 end-mark — SOFTENED</div>
        <div style={line}>
          <span>you finish my sentences</span>
          {heart(true)}
        </div>
        ...
        <div
          style={{ borderTop: '1px dashed #d9d4c8', margin: '18px 0 10px' }}
        />

        <div style={cap}>teal underline — CURRENT</div>
        <div style={line}>
          <span>i fear i have {feelings(false)}</span>
        </div>
        <div style={{ ...cap, marginTop: 10 }}>teal underline — SOFTENED</div>
        <div style={line}>
          <span>i fear i have {feelings(true)}</span>
        </div>
        ...
        <div style={cap}>softened marks beside the neighbour doodles</div>
        <div style={{ ...line, gap: 14, flexWrap: 'wrap' }}>
          {heart(true)}
          {chip('doodle-underline-teal-soft.png', 12)}
          {chip('doodle-oval-lavender.png')}
          {chip('doodle-rays-mustard.png')}
          {chip('doodle-exclaim-question.png')}
          {chip('doodle-wavy-underline.png', 12)}
        </div>
        <div style={{ ...cap, marginTop: 4 }}>
          soft&lt;3 · softteal · oval · rays · !? · wavy
        </div>
      </div>
    </div>
  );
...
export default function GiftPreviewPage({
  ...
  if (slug === 'love-receipt' && searchParams.view === 'softcompare') {
    return <SoftCompare />;
  }
