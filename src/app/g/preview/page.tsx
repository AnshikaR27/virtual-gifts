...
// font, dark outline, and a HARD (0-blur) offset drop shadow — a Language-A
// pixel shadow, never soft/blurred. Real body/band/subtitle/doodles unchanged
// (preview-only headerPreview prop). Reached with
...
function HeaderShadow() {
  ...
  // HARD offset shadow — 0 blur, dark, down-right. Language A (never blurred).
      ...
      <div
        style={{
          ...
          ...(opts.outline
            ...
            : {}),
          ...(opts.shadow ? { textShadow: HARD_SHADOW } : {}),
        }}
      >
        DELULU MART
      </div>
  ...
  const cards: { key: string; title: string; node: ReactNode }[] = [
    {
      key: 'A',
      title: 'CURRENT — Archivo Black (baseline)',
      node: <ReceiptPaper payload={payload} />,
    },
    {
      key: 'B',
      title: 'Fredoka + dark outline, NO shadow — 28px',
      node: (
        <ReceiptPaper
          payload={payload}
          headerPreview={{
    ...
    {
      key: 'C',
      title: 'Fredoka + outline + HARD offset shadow (the target) — 28px',
      node: (
        <ReceiptPaper
          payload={payload}
          headerPreview={{
    ...
    {
      key: 'D',
      title: 'Lilita One + outline + HARD offset shadow (alt font) — 30px',
      node: (
        <ReceiptPaper
          payload={payload}
          headerPreview={{
    ...
    >
      ...
      <div style={{ textAlign: 'center', maxWidth: 470 }}>
        <h1 style={{ fontSize: 16, margin: '0 0 6px', color: '#1a1a1a' }}>
          Love Receipt — rounded logo + HARD sticker shadow
        </h1>
        <p style={{ fontSize: 12, color: '#555', margin: 0, lineHeight: 1.5 }}>
          Real receipt; only the logo changes A→D. The shadow is a HARD 0-blur
          offset (3px down-right) — a Language-A pixel shadow, never blurred.
          Band + subtitle + body + doodles unchanged. All rounded options fit
          DELULU MART on one line at 390px (and down to 320px).
        </p>
      </div>
...
export default function GiftPreviewPage({
  ...
  // HEADERSHADOW (exploratory) — rounded logo + outline + hard offset shadow:
  // /g/preview?slug=love-receipt&view=headershadow
