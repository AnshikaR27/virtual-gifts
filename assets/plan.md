...
| Love Receipt: coverage cap rule-only; drop adjacency/no-repeat | 2026-07-12 | Author hand-authors each line's doodle via LINE_BINDINGS. Coverage cap (~40% of lines) RETAINED but applied to rule-engine fallback marks ONLY — hand bindings are unlimited and exempt (never counted/trimmed), keeping unbound draws calm while all 94 get hand-bound. no-adjacent-double and no-repeat-doodle removed. Per-line rule self-limiting kept | Active  |
